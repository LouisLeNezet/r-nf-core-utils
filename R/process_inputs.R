...
#'
#' This function processes input parameters and files, applying parameter
#' overrides, validating expected keys and files, and ensuring that the
#' provided file paths are valid.
#'
#' @param opt A list of default options.
#' @param args A character vector of command-line arguments to override
#' defaults.
#' @param keys_to_nullify A character vector of keys that should be nullified.
#' @param expected_files A character vector of keys in `opt` that are
#' expected to be file paths.
#' @param expected_folders A character vector of keys in `opt` that are
#' expected to be folder paths.
#' @param expected_double A character vector of keys in `opt` that are
...
process_inputs <- function(
  ...
  opt[expected_files] <- lapply(expected_files, function(x) {
    validate_file(opt[[x]], x)
  })
  opt[expected_folders] <- lapply(expected_folders, function(x) {
    validate_folder(opt[[x]], x)
  })

  # Check numeric inputs
  opt[expected_double] <- lapply(expected_double, function(x) {
    validate_double(opt[[x]], x)
  })
  ...
  if (!is.null(required_opts)) {
    missing <- required_opts[
      !unlist(lapply(opt[required_opts], is_valid_string)) |
